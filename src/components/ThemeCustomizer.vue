      ...
      <div class="mb-4">
        ...
        <div class="grid grid-cols-4 gap-2">
          <button v-for="color in colors" :key="color.name" class="flex flex-col items-center gap-1 rounded-md border p-1 text-[10px] transition-colors hover:bg-accent" :class="{ 'ring-2 ring-primary ring-offset-1': selectedColor === color.name }" @click="setColor(color.name)">
            <span class="h-6 w-6 rounded-full shadow-sm" :style="{ backgroundColor: color.value }"></span>
...
// 更新颜色变量 - 辅助函数
const updateColorVariable = (varName, hexColor) => {
  const rgbValues = getRGBValues(hexColor)
  document.documentElement.style.setProperty(varName, rgbValues)
}
...
const setColor = (color) => {
  ...
  const rgbValues = getRGBValues(colorValue)
